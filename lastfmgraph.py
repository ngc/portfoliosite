...
django.setup()
from blog.models import LastFMGraph
...
def get_tracks(server, usernames, startpage=1, sleep_func=time.sleep, tracktype='recenttracks'):
    ...
    for username in username_list:
        ...
        page = startpage
        response = connect_server(server, username, page, tracktype)
        ...
        while page <= totalpages:
            ...
            for trackelement in tracklist:
                ...
                if "nowplaying" not in trackelement.attrib or not trackelement.attrib["nowplaying"]:
                    total.append(graph_counter + 1)
                    timeseries.append(datetime.utcfromtimestamp(int(trackelement.find('date').get('uts'))))
                    graph_counter += 1
            ...
            page += 1
            if(page > totalpages):
                plt.plot(timeseries, total[::-1], label=username)

    plt.gcf().autofmt_xdate()
    plt.legend(loc="upper left")
    plt.title(graph_title)
    plt.savefig('file.png', dpi=300, bbox_inches='tight',pad_inches=0)
    LastFMPicture = LastFMGraph.objects.get(title="MASTER")
    LastFMPicture.picture.save('file.png', File(open('file.png', 'rb')))
    LastFMPicture.save()
    quit()
